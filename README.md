...
登录恢复成功后，若该站点配置了带 `action` 或 `newApiCheckin` 的 `nativeChallengePreflight`，自动复查会先接续原生签到，并读取同站点的当天确认。只有登录成功而没有签到证据时会保留人工待确认，不再切回另一种浏览器会话反复判定未登录。未配置原生签到的站点保持原有恢复流程。

原生按钮或链接入口可以等待延迟加载，实际点击后重新开始完整的结果确认等待，不使用猜测路径代替可见入口。预热项可用 `maxAttempts: 1` 限制重开窗口；默认最多两次。明确的雷池“客户端异常，请确认您是合法用户”会标记为交互验证并停止重复原生预热，不能作为正文已加载或签到成功证据。
