...
2. Bookmark scope, before every other preference: Chrome or Edge; profile name or Auto; parent/container folder name or names; target child-folder name or names. Offer candidates found by preflight, but do not preselect or infer names.
...
Do not ask for passwords, cookies, tokens, PINs, passkeys, or recovery codes in chat. If login is needed, open the isolated browser profile visibly and let the user complete it, or reuse the selected browser's encrypted saved-login database with their approval.
