import { normalizeAgentRouterAccountKey } from "./result-identity.mjs";
...
export function logicalCompletionKey(result, logicalGroups = {}) {
  ...
  const rawAccountKey = result?.accountKey;
  if (rawAccountKey == null || String(rawAccountKey).trim() === "") return String(group);
  try {
    const accountKey = normalizeAgentRouterAccountKey(rawAccountKey, "logical check-in accountKey");
    return `${String(group)}#account=${encodeURIComponent(accountKey)}`;
  } catch {
    return null;
  }
}
